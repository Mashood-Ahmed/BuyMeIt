...
const Navbar = ({ toggle }) => {
  ...
  const BarToggler = (e) => {
    ...
    const product = Products1.concat(Products2).filter((prod) => {
  ...
  const getSearch = (e) => {
    ...
    if (inp.length > 0) {
      const filteredCharacters = _Categories.filter((character) => {
        return character.startsWith(inp);
      });
    ...
    <Container>
          ...
          {onSearch && searchResults.length > 0 && (
            <SearchResults>
              {searchResults.map((result, i) => (
                <Result onClick={BarToggler} key={i} value={result}>
                  {result}
                </Result>
              ))}
            </SearchResults>
      ...
      <SubNav>
        ...
        <List>
          ...
          <ListItem>
            <Page to="/">Customer Service</Page>
          </ListItem>
          <ListItem>
            <Page to="/">Promo Codes</Page>
          </ListItem>
          <ListItem>
            <Page to="/">Services</Page>
          </ListItem>
        </List>
      </SubNav>
    </Container>
